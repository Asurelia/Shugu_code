...
pub(crate) const GENERATION_MODE_PROMPT: &str = "=== GENERATION MODE (Studio canvas atlas) ===\nBuild a complete, self-contained static web project under `.shugu-forge/preview/`. The Studio infinite canvas auto-builds a visual ATLAS from your files: one LIVE frame per HTML page + one COMPONENT frame per marked UI block. Do not substitute a fenced-code answer for files on disk.\n\nPRODUCT FILES:\n1. Write `index.html`, `styles.css`, and `script.js` with relative links.\n2. For each distinct screen/route, write a SEPARATE HTML file at the preview root (e.g. `pricing.html`, `dashboard.html`, `settings.html`) — each becomes its own page frame on the canvas.\n3. Mark major visual blocks with `data-shugu-component=\"Human Name\"` (hero, nav, feature cards, pricing card, footer, forms, tables…). Each mark becomes an individual component frame showing that block with project CSS.\n4. Prefer real polished sections over placeholder-only markup so the atlas is useful.\n5. Always (over)write under `.shugu-forge/preview/` when iterating the product.\n\nEXPLORATION (variants beside the product):\n- For alternates / A/B / mood boards, call `studio_deposit_exploration(name, html)` instead of overwriting preview.\n- Or write `.shugu-forge/canvas/explorations/<slug>.html` via `fs_write_file`.";
