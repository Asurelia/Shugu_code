...
- *Corroboration web (RE 2026)* : `cowork-svc.exe` ↔ pipe nommé `\\.\pipe\cowork-vm-service` (JSON-RPC, **mTLS** → seul `claude.exe` signé peut *commander* la VM). ⚠️ **Fuite connue** : tout process local peut *s'abonner* au flux d'events (stdout/stderr/fichiers) → **leçon Shugu** : authentifier les *abonnés* aux events Tauri, pas seulement les *émetteurs*. Image VM ~2,5 Go (CDN) → ~13 Go décompressée.
...
- **Sandbox Windows = comptes dédiés + token restreint + DACL NTFS + WFP** (modèle **en couches** ; PAS intégrité, PAS AppContainer ; confirmé par le blog OpenAI + writeups RE + imports PE locaux) :
  - `codex-windows-sandbox-setup.exe` (élevé, franchit l'UAC) crée **2 comptes locaux dédiés** `CodexSandboxOffline`/`Online` (`NetUserAdd`) + groupe (`NetLocalGroupAdd`/`AddMembers`) ; mots de passe **chiffrés DPAPI liés au profil du dev** dans `.sandbox-secrets/sandbox_users.json`.
  - **Écritures = deny-by-default** : le compte sandbox n'a **aucun write** par défaut ; `SetNamedSecurityInfoW` pose des **write-ACE** sur workspace + `writable_roots` (+ `Temp`) + un capability/synthetic-SID. **Lectures = largement ouvertes** (best-effort : profil, Windows, Program Files, ProgramData). ⇒ l'isolation des secrets vient du **compte distinct** (il ne peut **ni déchiffrer la DPAPI du dev ni lire les fichiers à ACL restrictive**), PAS d'un blocage des lectures.
  ...
  - **Exécution** : `codex.exe` (non-élevé) envoie un `SpawnRequest` via **pipe nommé** au helper ; `codex-command-runner.exe` (copié versionné dans `.sandbox-bin/`, ici `0.138.0-alpha.7`) lance la commande via **`CreateProcessAsUserW`** avec un **token restreint** dont le principal est le compte sandbox (+ `AdjustTokenPrivileges`). ⚠️ Lourd (UAC, comptes, DPAPI, WFP, helper) **mais invisible à l'usage** ; ce n'est PAS Docker.
...
| Exécution / sandbox | microVM Hyper-V | **comptes+token restreint+DACL+WFP** (`CreateProcessAsUserW`) | aucun OS | **MIC-low livré** (writes confinés ; reads/réseau ouverts) | 🟠 | OK comme 1er pas léger ; v2 = comptes dédiés si **secrets-illisibles** + net-gate voulus |
