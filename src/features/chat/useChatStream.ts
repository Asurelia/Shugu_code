    ...
    void (async () => {
      try {
        unlisten = await listen<ChatDelta>("chat://delta", (delta) => {
          if (cancelled) return;
          // Les appels `chat_send` HORS-FLUX (éditions inline `aiedit:*`,
          // synthèse vocale `speak:*`) réutilisent le même bus de deltas mais
          // ne doivent PAS alimenter le streaming du chat principal — sinon leur
          // `done` reset STREAM_KEY et coupe le stream d'un vrai message en cours
          // (bug de concurrence identifié en revue). Un helper unique couvre tous
          // les préfixes hors-flux d'un seul geste.
          if (isOutOfBandConvId(delta.conversationId)) return;
          diagEveryN(
            "chat-stream",
