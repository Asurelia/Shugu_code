// Shugu Forge — Tweaks panel + control kit.
// Ported from tweaks-panel.jsx. localStorage replaces the host postMessage protocol.
...
const STORAGE_KEY = "shugu.tweaks.v1";

// ── useTweaks ───────────────────────────────────────────────────────────────
// Persists merges into localStorage and broadcasts a `tweakchange` CustomEvent
// for in-page listeners (rail thumbnails, theme bootstrap).
...
export function TweaksPanel({ title = "Tweaks", children }: { title?: string; children?: React.ReactNode }) {
  ...
  // Open with Ctrl/Cmd+, — replaces the host-driven activate protocol.
  useEffect(() => {
...
// ── Layout helpers ──────────────────────────────────────────────────────────
export function TweakSection({ label, children }: { label?: string; title?: string; children?: React.ReactNode }) {
  return (
    <>
...
function CheckGlyph({ light }: { light: boolean }) {
  return (
        ...
        {options.map((o: any, i: number) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" role="radio"
              ...
              {on && <CheckGlyph light={isLight(hero)} />}
            </button>
          );
        })}
      </div>
...
// Re-export useMemo so the panels file can import everything from one spot.
export { useMemo };
