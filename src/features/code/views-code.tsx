...
// ─── Terminal (mock, real xterm wiring later) ───────────────
export function TerminalView() {
  const [lines, setLines] = useState<any[]>([
    { t: "out", text: "Shugu Forge Terminal · bash · session #1" },
    { t: "out", text: "Last login: Mon May 11 14:22 — 12 modified files" },
    { t: "prompt", text: "cargo run --release" },
    { t: "out", text: "   Compiling shugu-forge v0.4.0 (./src-tauri)" },
    { t: "out", text: "   Compiling tauri v2.1.1" },
    { t: "warn", text: "warning: unused variable: `app`" },
    { t: "ok", text: "    Finished `release` profile [optimized] target(s) in 18.42s" },
    { t: "out", text: "     Running `target/release/shugu-forge`" },
    { t: "ok", text: "[tauri] window 'main' ready · veil-shell loaded · 0 errors" },
  ]);
  ...
  const submit = (e: React.KeyboardEvent) => {
    ...
    if (!cmd) return;
    const next = [...lines, { t: "prompt", text: cmd }];
    if (cmd.startsWith("ls")) {
      next.push({ t: "out", text: "src/  src-tauri/  public/  package.json  vite.config.ts  tsconfig.json" });
    } else if (cmd.startsWith("git status")) {
      next.push({ t: "out", text: "On branch main" });
      next.push({ t: "ok", text: "  modified:   src/components/Forge.tsx" });
      next.push({ t: "warn", text: "  new file:   src/views/ImageView.tsx" });
    } else if (cmd === "clear") {
      setLines([]); setInput(""); return;
    } else if (cmd.startsWith("shugu")) {
      next.push({ t: "ok", text: "→ launching shugu agent…" });
    } else {
      next.push({ t: "err", text: `command not found: ${cmd}` });
    }
    setLines(next);
    setInput("");
  };
  ...
  return (
    <div className="term-shell">
      <div className="term-tabs">
        <button className="term-tab active"><span style={{width:6,height:6,borderRadius:"50%",background:"var(--tertiary)"}}></span> bash · shugu-forge</button>
        <button className="term-tab">node · dev-server</button>
        <button className="term-tab">tauri · build</button>
        <span style={{flex:1}}></span>
        <button className="composer-tool" title="Split"><Icon name="diff" size={13}/></button>
        <button className="composer-tool" title="New"><Icon name="plus" size={13}/></button>
      </div>
      <div className="term-feed" ref={feedRef}>
        {lines.map((l, i) => (
          <div key={i} className={"term-line " + (l.t !== "prompt" ? l.t : "")}>
            {l.t === "prompt"
              ? <><span className="prompt">shugu ❯</span> <span className="cmd">{l.text}</span></>
              : l.text}
          </div>
        ))}
      </div>
      <div className="term-input">
        <span className="prompt">shugu ❯</span>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={submit} placeholder="type a command (try: git status, ls, shugu lint)" autoFocus/>
      </div>
    </div>
  );
}
